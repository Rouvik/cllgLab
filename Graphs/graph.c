...
void Graph_addEdge(int* adj, int v1, int v2, bool bidirectional)
{
	...
	if(bidirectional) INDEX_ADJ(adj, v2, v1) = 1;
}
...
bool Graph_cycleDetect(int *adj, int v, int *visited)
{
	if(v > adj[0])
	{
	...
	for(int i = 0; i < adj[0]; i++)
	{
		if(i == v) continue;	// dont try to visit self

		if(INDEX_ADJ(adj, i, v))
		{
			if(visited[i]) return true;
			detected |= Graph_cycleDetect(adj, i, visited);
		}
	}
...
int main(int argc, const char *argv[])
{
	...
	printf("Has cycles? %s\n", Graph_cycleDetect(graph, 0, visited) ? "TRUE" : "FALSE");
